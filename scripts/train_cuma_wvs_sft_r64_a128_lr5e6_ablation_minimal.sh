...
# When demographic_embed_dim is 0, the embedding model is not loaded, but the argument is still required.
# We can use a placeholder or keep it as is.
EMBEDDING_MODEL_PATH="../MyModels/Qwen3-Embedding-0.6B"
DATASET_PATH="./data/wvs_sampled_sft/sampled_50k/train"

# Experiment Name: Ablation study removing Load Balancing, Demographic Embeddings, and System Prompts
EXP_NAME="cuma_wvs_50k_r64_a128_lr5e6_ablation_minimal"
...
if [ "$EVAL_ONLY" = false ]; then
    echo "Starting training (Ablation: No Balance, No Demo Embed, No System Prompt)..."
    accelerate launch \
        ...
        --save_steps 999999 \
        --seed 42 
        # Removed --use_load_balancing_loss
        # Removed --lambda_auxiliary 0.01
        # Set --demographic_embed_dim 0
        # Removed --use_demographic_system_prompt
fi
...
accelerate launch \
    ...
    --adapter_type cuma
    # Removed --use_demographic_system_prompt from eval as well
